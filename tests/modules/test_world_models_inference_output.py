...
def test_recurrent_state_batch_unbatch_none():
    """Verifies batching logic when there is no recurrent memory."""
    state1 = {"dynamics": torch.randn(1, 4), "wm_memory": None}
    state2 = {"dynamics": torch.randn(1, 4), "wm_memory": None}

    batched = batch_recurrent_state([state1, state2])
...
def test_recurrent_state_batch_unbatch_tensor():
    """Verifies batching logic for standard tensor recurrent memory."""
    state1 = {"dynamics": torch.randn(1, 4), "wm_memory": torch.randn(1, 8)}
    state2 = {"dynamics": torch.randn(1, 4), "wm_memory": torch.randn(1, 8)}
...
def test_recurrent_state_batch_unbatch_tuple_lstm_style():
    ...
    state1 = {"dynamics": torch.randn(1, 4), "wm_memory": (h1, c1)}
    state2 = {"dynamics": torch.randn(1, 4), "wm_memory": (h2, c2)}
...
def test_recurrent_state_batch_unbatch_dict():
    ...
    state1 = {"dynamics": torch.randn(1, 4), "wm_memory": d1}
    state2 = {"dynamics": torch.randn(1, 4), "wm_memory": d2}

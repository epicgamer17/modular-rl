    ...
    def backpropagate(
        ...
        for i in range(n - 1, -1, -1):
            ...
            if i > 0:
                ...
                # Update parent's child values (for UCB scoring)
...
def test_min_max_stats_parity():
    """Compare VectorizedMinMaxStats against scalar MinMaxStats."""
    batch_size = 1
...
def test_ucb_scoring_parity(config):
    """Compare ucb_score_fn against UCBScoring."""
    device = torch.device("cpu")
    num_actions = 4
    batch_size = 1
    parent_visits = 10
...
def test_v_mix_parity(config):
    """Compare aos compute_v_mix against search_py DecisionNode.get_v_mix."""
    device = torch.device("cpu")
    num_actions = 4
    batch_size = 1
    parent_v_net = 0.5
...
def test_gumbel_scoring_parity(config):
    """Compare gumbel_score_fn against GumbelScoring."""
    device = torch.device("cpu")
    num_actions = 4
    batch_size = 1
    parent_v_net = 0.5
...
def test_backprop_average_parity(config):
    """Compare average_discounted_backprop against AverageDiscountedReturnBackpropagator."""
    device = torch.device("cpu")
    num_actions = 4
    batch_size = 1
...
class MockNetwork(torch.nn.Module):
    ...
    def obs_inference(self, obs):
        B = obs.shape[0] if isinstance(obs, torch.Tensor) else len(obs)
        if isinstance(obs, list):
            obs = torch.stack(obs)
        value = obs.view(B, -1).mean(dim=-1).float()
        logits = torch.zeros((B, self.num_actions), dtype=torch.float32)
        logits[:, 0] = 1.0
        return SimpleNamespace(
            value=value,
    ...
    def hidden_state_inference(self, state, action):
        ...
        if action.dim() > 1:
            ...
            action_val = action
        new_state_data = state_data + action_val.view(B, 1, 1, 1) * 0.1
        value = new_state_data.view(B, -1).mean(dim=-1).float()
        logits = torch.zeros((B, self.num_actions), dtype=torch.float32)
        logits[:, 0] = 1.0
        return SimpleNamespace(
            value=value,
...
def test_full_search_stochastic_parity(config):
    ...
    net.afterstate_inference = afterstate_inference
    obs = torch.ones((batch_size, 1, 4, 4))
    info = {"legal_moves": [list(range(num_actions))]}
    py_info = {"legal_moves": info["legal_moves"][0]}
    from search.aos_search.search_factories import build_search_pipeline
...
def run_all():
    ...
    for test in tests:
        ...
        try:
            if test.__name__ == "test_min_max_stats_parity":
                test()
            else:
                test(cfg)
            print("PASSED")

...
def test_replay_collator_correct_shapes_and_dtypes():
    ...
    # Check shapes
    assert collated["obs"].shape == (batch_size, obs_dim), f"Obs shape mismatch: {collated['obs'].shape}"
    assert collated["action"].shape == (batch_size,), f"Action shape mismatch: {collated['action'].shape}"
    assert collated["reward"].shape == (batch_size,), f"Reward shape mismatch: {collated['reward'].shape}"
    assert collated["done"].shape == (batch_size,), f"Done shape mismatch: {collated['done'].shape}"
...
def test_replay_collator_partial_schema():
    ...
    assert "extra" in collated
    assert collated["extra"] == ["a", "b"]

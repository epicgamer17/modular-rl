...
class MockNetwork(nn.Module):
    ...
    def obs_inference(self, obs):
        ...
        return InferenceOutput(policy=Categorical(logits=torch.ones((1, 2))), value=torch.tensor([0.0]))
    def eval(self): pass
    def state_dict(self): return {}
    def load_state_dict(self, d, strict=True): pass
    def parameters(self): return [nn.Parameter(torch.zeros(1))]
...
class MockMultiplayerAdapter:
    ...
    def step(self, actions):
        ...
        return torch.zeros((1, 4), device=self.device), reward, terminated, truncated, info
...
def test_base_trainer_score_stats_recording():
    ...
    res = {
        "batch_scores": [np.array([25.0, -10.0])],
        "batch_lengths": [10]
    }
...
if __name__ == "__main__":
    test_training_score_aec_multi_reward()
    test_base_trainer_score_stats_recording()

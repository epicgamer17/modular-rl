...
def test_actor_trajectory_payload_contract():
    ...
    assert captured_sequence is not None, "Actor should have flushed the completed sequence to the buffer."
    ...
    expected_keys = [
        "observations", "actions", "rewards", "policies", "values", "to_plays"
    ]
    ...
    assert torch.allclose(payload["policies"][0], target_policy_0[0]), "Policy at step 0 should match MCTS target_policies."
    # Step 1: policy was target_policy_1
    assert torch.allclose(payload["policies"][1], target_policy_1[0]), "Policy at step 1 should match MCTS target_policies."
    ...
    print("\n[Trajectory Payload Contract Verified]")

...
def test_optimizer_tracks_real_model():
    ...
    assert agent.q_net is ctx.get_model(config.model_handle), "Context must return the SAME model object as the agent"
    ...
    assert model_param is opt_param, "Optimizer must track the SAME tensor objects as the model"
    ...
    print("\n[Success] Optimizer correctly updated the live model weights.")
...
def test_compiler_does_not_break_references():
    ...
    assert ctx.get_model(config.model_handle) is initial_model, "Compiler broke model reference"
    assert ctx.get_optimizer("main_opt") is initial_opt, "Compiler broke optimizer reference"

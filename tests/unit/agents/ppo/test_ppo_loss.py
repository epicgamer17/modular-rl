...
def test_ratio_equals_one_when_same_policy():
    ...
    gae = {"advantages": torch.ones(steps), "returns": torch.zeros(steps)}

    node = Node(
        "ppo",
        "PPO_Objective",
        params={"clip_epsilon": 0.2, "normalize_advantages": False},
    )
...
def test_clip_bounds_ratio():
    ...
    gae = {"advantages": torch.tensor([1.0]), "returns": torch.zeros(steps)}
...
def test_entropy_positive():
    ...
    gae = {"advantages": torch.zeros(steps), "returns": torch.zeros(steps)}

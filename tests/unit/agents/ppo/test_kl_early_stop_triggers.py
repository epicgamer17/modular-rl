...
def test_kl_early_stop_triggers():
    ...
    with patch("runtime.engine.LearnerRuntime.update_step") as mock_update:
        ...
        # With 2 minibatches per epoch, and KL > target_kl after 1st epoch:
        # It should run 1 epoch (2 minibatches) and then stop.
        # Total calls should be 2.
        assert mock_update.call_count == 2, f"Expected 2 calls (1 epoch), got {mock_update.call_count}"

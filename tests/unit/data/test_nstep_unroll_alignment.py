...
def test_nstep_unroll_toplay_alignment():
    ...
    rewards = (
        torch.arange(max_size).float() * 10
    )  # r1=10, r2=20, ... (r_i is reward for s_{i-1}->s_i)
    ...
    assert (
        target_rewards[1] == 100.0
    ), f"r1 mismatch: expected 100, got {target_rewards[1]}. Logic: if s0 is at index 10, r1 must be rewards[10]."
    assert (
        target_rewards[2] == 110.0
    ), f"r2 mismatch: expected 110, got {target_rewards[2]}"
...
def test_nstep_unroll_terminal_masking():
    ...
    assert (
        tp_mask[2] == False
    ), f"s2 (post-terminal) must be masked out. Mask: {tp_mask}"
...
def test_nstep_unroll_same_game_masking():
    ...
    game_id = torch.zeros(100, dtype=torch.long)
    game_id[10] = 7
    game_id[11] = 7
    game_id[12] = 8  # Global change at index 12
    ...
    assert (
        is_same_episode[2] == False
    ), f"Index 2 (buffer 12) is game 8, but base was 7. same_game: {is_same_episode}"

    assert tp_mask[1] == True  # s1 is same game
    assert (
        tp_mask[2] == False
    ), f"s2 (different game) must be masked out. Mask: {tp_mask}"

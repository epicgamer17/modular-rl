...
def test_muzero_turn_flip():
    ...
    with torch.inference_mode():
        child1_output = network.hidden_state_inference(
            network_state=root_output.network_state, action=action
        )
    ...
    with torch.inference_mode():
        child2_output = network.hidden_state_inference(
            network_state=child1_output.network_state, action=action2
        )
...
def test_muzero_value_perspective():
    ...
    num_actions = 9
    network = make_muzero_network(
        obs_dim=obs_dim, num_actions=num_actions, resnet_filters=[8]
    )
    network.eval()
    ...
    assert (
        diff > 0 or network.components["prediction_backbone"][0].weight.std() > 0
    ), "Network might be initialized to constant"
...
def test_muzero_p1_win_targets():
    ...
    buffers = {
        ...
        "episode_ids": torch.ones(seq_len, dtype=torch.int64),
        "step_ids": torch.arange(seq_len, dtype=torch.int32),
    ...
    assert (
        rewards[0, 2] == 1.0
    ), f"P1 winning reward (r2) should be 1.0 at index 2, got {rewards[0, 2]}"
    ...
    ), f"P1 state value target should be 1.0, got {target_values[0, 1]}"
    ...
    ), f"P0 state value target should be -1.0, got {target_values[0, 0]}"

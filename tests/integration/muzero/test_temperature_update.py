...
class MockNetwork(torch.nn.Module):
    ...
    def obs_inference(self, obs):
        ...
        return InferenceOutput(
            policy=Categorical(logits=logits), value=torch.tensor([0.0])
        )
...
def test_temperature_selector_training_step_flow():
    ...
    cfg = ScheduleConfig(
        type="stepwise", steps=[100], values=[1.0, 0.1], with_training_steps=True
    )
...
def test_temperature_selector_episode_step_flow():
    ...
    cfg = ScheduleConfig(
        type="stepwise", steps=[10], values=[1.0, 0.1], with_training_steps=False
    )

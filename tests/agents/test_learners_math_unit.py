...
def test_rainbow_learner_math(rainbow_cartpole_replay_config):
    """
    Test RainbowLearner loss calculation.
    """
    torch.manual_seed(42)
    device = torch.device("cpu")
    num_actions = 2
    ...
    # 1. Use real config from conftest.py
    config = rainbow_cartpole_replay_config

    # 2. Instantiate networks and learner
    online_net = ModularAgentNetwork(config, obs_dim, num_actions)
    target_net = ModularAgentNetwork(config, obs_dim, num_actions)
    learner = RainbowLearner(
        config, online_net, target_net, device, num_actions, obs_dim, torch.float32
    )
    ...
    # 4. Directly call internal loss math
    loss, elementwise_loss = learner.compute_loss(batch)

    # 5. Assert valid losses
    assert not torch.isnan(loss)
    assert loss.ndim == 0  # scalar
    assert elementwise_loss.shape == (2,)  # per-sample loss

...
def _process_single_replay(raw: dict) -> Optional[dict]:
    """Uses GodModeStepper (Path A) to generate flawless dataset observations."""
    reset_parser_state()
    ...
    stepper = GodModeStepper(representation="image")
    ...
    for event in events:
        ...
        # --- DRAIN PENDING DISCARDS DEMANDED BY CATANATRON ---
        # Because Colonist resolves discards simultaneously and out-of-order
        # with the robber move, we must forcefully clear Catanatron's strict
        # discard stack, saving the mandatory transitions into the dataset.
        while True:
            env = stepper.env.unwrapped
            stepper._update_true_bank()
            stepper._recount_vps()
            ...
            # If Catanatron demands a discard to proceed:
            if len(valid_actions) == 1 and DISCARD_IDX in valid_actions:
        ...
        if result is not None:
            # Filter out DISCARD actions coming from Colonist since we auto-drained them above
            filtered_result = [

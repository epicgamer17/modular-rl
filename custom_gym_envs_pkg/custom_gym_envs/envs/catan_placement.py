...
class CatanPlacementAECEnv(CatanAECEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.include_roads_in_action_space:
            self.placement_actions = PLACEMENT_SETTLEMENT_ACTIONS + PLACEMENT_ROAD_ACTIONS
        else:
    ...
    def _get_action_from_placement_index(self, index: int, playable_actions: List[Action]) -> Action:
        action_type, value = self.placement_actions[index]
        for action in playable_actions:
            norm = normalize_action(action)
            if norm.action_type == action_type and norm.value == value:
                return action
        raise ValueError(f"Action index {index} ({action_type}, {value}) not found in playable_actions")

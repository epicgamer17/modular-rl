...
def op_metrics_sink(node: Node, inputs: Dict[str, Any], context: Optional[ExecutionContext] = None) -> Dict[str, Any]:
    """
    ...
    if replay_size is None:
        ...
        rb = context.get_buffer(buffer_id) if context else node.params.get("replay_buffer")
        if rb is not None:
            replay_size = len(rb)
    ...
    if context.learner_step % log_frequency == 0:
        ...
        print(f"[Metrics] Step {context.actor_step} | Learner {context.learner_step} | "

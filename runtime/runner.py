"""
Execution loops for the RL IR runtime.
Handles periodic tasks, main execution loops, and rollout pooling.
"""
...
class ScheduleRunner:
    ...
    def _perform_target_sync(self, context: ExecutionContext, on_type: str = "learner_step"):
        ...
        Composite learners (e.g. NFSP) that don't expose a single `train_graph`
        manage their own sync nodes internally — skip the global scan for them.
        """
        from core.graph import NODE_TYPE_TARGET_SYNC

        train_graph = getattr(self.learner_runtime, "train_graph", None)
        if train_graph is None:
            return

        sync_nodes = [nid for nid, node in train_graph.nodes.items()

...
def build_actor_graph(config: DQNConfig) -> Graph:
    ...
    graph.add_node(
        "obs_in", 
        NODE_TYPE_SOURCE, 
        schema_out=Schema([Field("obs", obs_spec)])
    )
    
    # 1b. Source for clock (to satisfy reachability)
    graph.add_node("clock_in", NODE_TYPE_SOURCE, schema_out=Schema([Field("clock", TensorSpec(shape=(), dtype="int64"))]))
...
def build_learner_graph(config: DQNConfig, collator) -> Graph:
    ...
    graph.add_node("td_loss", "TDLoss", params={"model_handle": config.model_handle, "target_handle": config.target_handle, "gamma": config.gamma})

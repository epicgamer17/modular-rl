...
class ReanalyzeActor(BaseActor):
    ...
    def reanalyze(self, batch_size: int) -> Dict[str, Any]:
        ...
        result = self.search_policy_source.get_inference(flat_obs, all_infos, agent_network=self.network)
...
class DAggerActor(RolloutActor):
    ...
    def __init__(
        ...
        """
        super().__init__(adapter, student_network, policy_source, buffer, num_players)
        ...
        while steps_this_call < num_steps:
            ...
            for i in range(self.num_envs):
                transition = {
                    ...
                    "value": result.value[i].item() if result.value is not None else None
                }
...
class NFSPActor(RolloutActor):
    ...
    def __init__(
        ...
        """
        super().__init__(adapter, rl_network, policy_source, buffer, num_players)
        ...
        while steps_this_call < num_steps:
            ...
            for i in range(self.num_envs):
                ...
                if terminals[i] or truncations[i]:
                    ...
                    self.seq_manager.append(i, {"observation": next_obs[i].cpu().numpy(), "terminated": False, "truncated": False})

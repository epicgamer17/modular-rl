...
import random
import numpy as np
from core.graph import Graph, NODE_TYPE_SOURCE, NODE_TYPE_ACTOR, NODE_TYPE_SINK, NODE_TYPE_TRANSFORM
...
def op_source(node, inputs):
    return None # Handled by initial_inputs

def op_q_actor(node, inputs):
...
def op_sample_batch(node, inputs):
    rb = node.params["buffer"]
...
def train_dqn(env_name="CartPole-v1", total_steps=2000):
    ...
    interact_graph.add_node("actor", "QActor", params={"q_net": q_net, "epsilon": 0.1, "act_dim": act_dim})
    ...
    train_graph = Graph()
    train_graph.add_node("sampler", "SampleBatch", params={"buffer": rb, "batch_size": 64})
    train_graph.add_node("loss", "TDLoss", params={"q_net": q_net, "target_net": target_net, "gamma": 0.99})
    ...
    for step in range(total_steps):
        # 1. Interaction
        res_interact = execute(interact_graph, initial_inputs={"obs_in": torch.tensor(obs, dtype=torch.float32)})

...
def op_mixture_actor(node, inputs, context=None):
    ...
    mode = "best_response" if rand_val < eta else "average_policy"
...
def run_nfsp_demo(total_steps=5000):
    ...
    interact_graph.add_node("actor", "MixtureActor", params={
        "q_handle": "online_q", "policy_handle": "policy", "eta": 0.1, "act_dim": act_dim
    })
    interact_graph.add_edge("obs_in", "actor", dst_port="obs")
    
    train_graph = Graph()
    # RL Sub-graph
    train_graph.add_node("rl_sampler", NODE_TYPE_REPLAY_QUERY, params={
        "buffer_id": "rl", "batch_size": 64, "min_size": 100, "collator": rl_collator
    })
    train_graph.add_node("rl_loss", "TDLoss", params={
        "q_handle": "online_q", "target_handle": "target_q", "gamma": 0.99
    })
    ...
    # SL Sub-graph
    train_graph.add_node("sl_sampler", NODE_TYPE_REPLAY_QUERY, params={
        "buffer_id": "sl", "batch_size": 64, "min_size": 100, "collator": sl_collator
    })
    ...
    train_graph.add_node("metrics", NODE_TYPE_METRICS_SINK, params={
        "log_frequency": 100, "buffer_id": "rl"
    })
    ...
    def nfsp_record(step_data):
        ...
        actor_val = step_data["metadata"]["actor_results"].get("actor")
        if actor_val and hasattr(actor_val, "data") and actor_val.data.get("mode") == "best_response":
            sl_buffer.add({
                "obs": step_data["obs"],
                "action": torch.tensor(actor_val.data["action"])
            })

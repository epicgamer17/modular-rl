...
def render_graph(
    ...
    Semantic node and edge colors come from styles.py (Actor/Transform/Source/...).
    Background, axis, label, and title colors track the active theme.
    """
    G = nx.DiGraph()
    ...
    with plot_theme(theme):
        text_color = plt.rcParams.get("text.color", "#D2DAE2")
        ...
        for nid, data in G.nodes(data=True):
            ...
            elif mode == "runtime" and params.get("active", False):
                state = "active"
        ...
        nx.draw_networkx_labels(G, pos, font_size=9, font_color=text_color, font_weight="bold")

        plt.title(
            title or f"Graph View [{mode.upper()}]",

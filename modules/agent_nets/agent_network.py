...
class AgentNetwork(nn.Module):
    ...
    def hidden_state_inference(self, network_state: Dict[str, Any], action: Tensor) -> InferenceOutput:
        ...
        if "memory_core" in self.components:
            seq_features = features.unsqueeze(1)
            seq_memory, next_h = self.components["memory_core"](seq_features, network_state.get("backbone_memory"))
            features = seq_memory.squeeze(1)
        else:
            next_h = None
    ...
    def afterstate_inference(self, recurrent_state: Any, action: Tensor) -> InferenceOutput:
        ...
        wm_output = self.components["world_model"].afterstate_recurrent_inference(recurrent_state, action)
